/**
 * Complete KYC (Know Your Customer) credential issuance and verification flow
 * This example demonstrates the full lifecycle of KYC verification on Stellar
 */
...
async function main() {
  ...
  try {
    ...
    const complianceResult = await sdk.performComplianceCheck(userKeypair.publicKey());
    console.log(`✅ Compliance Status: ${complianceResult.status}`);
    console.log(`   Risk Score: ${complianceResult.riskScore}/100`);
    console.log(`   Valid Credentials: ${complianceData.validCredentials}/${complianceData.totalCredentials}\n`);

    // Step 7: Create age verification using zero-knowledge proof
    console.log('🔒 Step 7: Creating Age Verification (ZK Proof)...');
    const ageCommitment = sdk.zkProofs.generateCommitment('1990-01-15', 'user_salt');
    const ageProofId = await sdk.zkProofs.createAgeProof(
      'age_verification',
      ageCommitment,
      18, // Prove user is over 18
      'mock_age_proof_bytes' // In reality, this would be generated by ZK circuit
    );
    console.log(`✅ Age proof created: ${ageProofId}\n`);

    // Step 8: Verify age proof
    console.log('🔐 Step 8: Verifying Age Proof...');
    const ageVerification = await sdk.zkProofs.verifyAgeProof(ageProofId, 18);
    ...
    const identityProfile = await sdk.getIdentityProfile(userKeypair.publicKey());
    ...
    setInterval(async () => {
      try {
        ...
        if (complianceResult.status === 'flagged' || complianceResult.status === 'blocked') {
          console.warn(`⚠️ Compliance alert for ${userAddress}: ${complianceResult.status}`);
